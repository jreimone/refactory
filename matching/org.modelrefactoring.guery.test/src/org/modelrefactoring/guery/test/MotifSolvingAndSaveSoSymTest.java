...
public class MotifSolvingAndSaveSoSymTest {

	private static String CONFIG_FILE_PATH		= "config/MotifSolvingAndSaveSoSymTest.config";
	...
	private static void initCSVPrinter() {
		...
		String statisticsFilePath = "statisticsSoSyM/matching_" + System.currentTimeMillis() + ".csv";
	...
	public void testAndGenerateRoleMappings() {
		...
		System.out.println("Original RoleModel: " + roleModel.getName() + " (R: " + roleCount + ", C: " + collCount + ")");
		System.out.println("Original Collaborations/Role: " + colPerRole);
		...
		for (org.modelrefactoring.guery.Motif motif : motifs) {
			...
			String resultFilePath = "rolemappingsSoSyM/" + metamodel.getName() + "_" + roleModel.getName() + "_MPL" + maxPathLength + "_SoSyM" + index + "." + new RolemappingMetaInformation().getSyntaxName();
			...
			if(csvPrinter != null){
				try {
					roleCount = motifByModel.getRoles().size();
					collCount = motifByModel.getPathRoles().size();
					colPerRole = (double) collCount / (double) roleCount;
					System.out.println("RoleModel without optional roles: " + roleModel.getName() + " (R: " + roleCount + ", C: " + collCount + ")");
					System.out.println("Collaborations/Role without optional roles: " + colPerRole);
	...
	private MotifModel convertRoleModel2MotifModel() {
		RoleModel2MotifConverterWithoutOptionalRoles converter = new RoleModel2MotifConverterWithoutOptionalRoles(roleModel);
		MotifModel motifModel = converter.createMotifModel(maxPathLength);
		assertEquals("motif model must only contain one motif", 1, motifModel.getMotifs().size());
		Resource roleModelResource = roleModel.eResource();
		ResourceSet rs = roleModelResource.getResourceSet();
		String gueryFileName = "queriesSoSyM/" + metamodelName + "_" + rolemodelName + "_MPL" + maxPathLength + "." + new GueryMetaInformation().getSyntaxName();
	...
	public EPackageGraphAdapter getGraphAdapter() {
		return new EPackageSoSyMGraphAdapter(metamodel);
	}
