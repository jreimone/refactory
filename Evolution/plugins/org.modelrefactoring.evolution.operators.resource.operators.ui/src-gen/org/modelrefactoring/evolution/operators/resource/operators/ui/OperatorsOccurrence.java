 ...
 */
public class OperatorsOccurrence {
	
	private static interface ITokenScannerConstraint {
		public boolean mustStop(org.modelrefactoring.evolution.operators.resource.operators.ui.IOperatorsTokenScanner tokenScanner);
	}
	
	public final static String OCCURRENCE_ANNOTATION_ID = "org.modelrefactoring.evolution.operators.resource.operators.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "org.modelrefactoring.evolution.operators.resource.operators.ui.occurences.declaration";
	 ...
	 */
	public OperatorsOccurrence(org.modelrefactoring.evolution.operators.resource.operators.IOperatorsTextResource textResource, ProjectionViewer projectionViewer) {
		super();
	...
	protected int getLength(org.modelrefactoring.evolution.operators.resource.operators.IOperatorsLocationMap locationMap, EObject eObject) {
	...
	public void updateOccurrenceAnnotations() {
		...
		org.modelrefactoring.evolution.operators.resource.operators.ui.IOperatorsTokenScanner tokenScanner = scan(referencedElement, new ITokenScannerConstraint() {
	...
	protected void addAnnotationsForReferences(EObject referencedElement, List<String> matchingNames) {
		...
		for (Setting setting : referencingObjects) {
			...
			for (String name : matchingNames) {
				...
				if (index > 0) {
					addAnnotation(document, org.modelrefactoring.evolution.operators.resource.operators.ui.OperatorsPositionCategory.PROXY, name, index, name.length());
				}
			}
		}
	}
	...
	protected List<String> addAnnotationsForDefinition(EObject referencedElement, String definitionText, int definitionOffset, final int caretOffset) {
		...
		if (definitionText == null) {
			...
			final List<String> names = new org.modelrefactoring.evolution.operators.resource.operators.analysis.OperatorsDefaultNameProvider().getNames(referencedElement);
			scan(referencedElement, new ITokenScannerConstraint() {
				...
				public boolean mustStop(org.modelrefactoring.evolution.operators.resource.operators.ui.IOperatorsTokenScanner tokenScanner) {
					...
					if (names.contains(text) && isContainedIn(offset, length, caretOffset)) {
						...
						addAnnotation(document, org.modelrefactoring.evolution.operators.resource.operators.ui.OperatorsPositionCategory.DEFINITION, text, offset, text.length());
					}
					return false;
				}
			});
			...
			addAnnotation(document, org.modelrefactoring.evolution.operators.resource.operators.ui.OperatorsPositionCategory.DEFINITION, definitionText, definitionOffset, definitionText.length());
	...
	protected int getIndexOf(EObject eObject, final String text) {
		org.modelrefactoring.evolution.operators.resource.operators.ui.IOperatorsTokenScanner tokenScanner = scan(eObject, new ITokenScannerConstraint() {
	...
	protected org.modelrefactoring.evolution.operators.resource.operators.ui.IOperatorsTokenScanner scan(EObject object, ITokenScannerConstraint constraint) {
		IDocument document = getSourceViewer().getDocument();
		
		org.modelrefactoring.evolution.operators.resource.operators.IOperatorsLocationMap locationMap = textResource.getLocationMap();
		
		org.modelrefactoring.evolution.operators.resource.operators.ui.IOperatorsTokenScanner tokenScanner = createTokenScanner();
	...
	protected org.modelrefactoring.evolution.operators.resource.operators.ui.IOperatorsTokenScanner createTokenScanner() {
		return new org.modelrefactoring.evolution.operators.resource.operators.ui.OperatorsUIMetaInformation().createTokenScanner(null, null);
	}
	
	protected void addAnnotation(IDocument document, org.modelrefactoring.evolution.operators.resource.operators.ui.OperatorsPositionCategory type, String text, int offset, int length) {
		...
		if (type == org.modelrefactoring.evolution.operators.resource.operators.ui.OperatorsPositionCategory.DEFINITION) {
	...
	protected void removeAnnotations() {
		removeAnnotations(org.modelrefactoring.evolution.operators.resource.operators.ui.OperatorsOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(org.modelrefactoring.evolution.operators.resource.operators.ui.OperatorsOccurrence.DECLARATION_ANNOTATION_ID);
	}
