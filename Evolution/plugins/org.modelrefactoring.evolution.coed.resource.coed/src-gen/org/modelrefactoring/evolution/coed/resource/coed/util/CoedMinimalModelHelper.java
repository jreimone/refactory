 ...
 */
public class CoedMinimalModelHelper {
	
	private final static org.modelrefactoring.evolution.coed.resource.coed.util.CoedEClassUtil eClassUtil = new org.modelrefactoring.evolution.coed.resource.coed.util.CoedEClassUtil();
			...
			if (feature instanceof EReference) {
				...
				if (type instanceof EClass) {
					...
					for (int i = 0; i < lowerBound; i++) {
						...
						if (reference.isContainment()) {
							...
							String initialValue = "#some" + org.modelrefactoring.evolution.coed.resource.coed.util.CoedStringUtil.capitalize(typeClass.getName());
						...
						if (value instanceof List<?>) {
							List<EObject> list = org.modelrefactoring.evolution.coed.resource.coed.util.CoedListUtil.castListUnchecked(value);
							list.add(subModel);
						} else {
				...
				if ("EString".equals(attribute.getEType().getName())) {
					...
					if (attribute.getName().equals("name") && name != null) {
						...
						initialValue = "some" + org.modelrefactoring.evolution.coed.resource.coed.util.CoedStringUtil.capitalize(attribute.getName());
					}
					Object value = root.eGet(attribute);
					if (value instanceof List<?>) {
						List<String> list = org.modelrefactoring.evolution.coed.resource.coed.util.CoedListUtil.castListUnchecked(value);
