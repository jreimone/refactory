 ...
 * </ul>
 */
public class CoedBracketPreferencePage extends PreferencePage implements IWorkbenchPreferencePage {
	
	private static final String[] ALL_LEFT_BRACKETS = new String[] { "{", "(", "[", "<", "\"", "'", };
	private static final String[] ALL_RIGHT_BRACKETS = new String[] { "}", ")", "]", ">", "\"", "'", };
	
	private String BRACKETS_COLOR = org.modelrefactoring.evolution.coed.resource.coed.ui.CoedPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR;
	...
	private String language = new org.modelrefactoring.evolution.coed.resource.coed.mopp.CoedMetaInformation().getSyntaxName();
	
	private org.modelrefactoring.evolution.coed.resource.coed.ui.CoedBracketSet bracketsTmp;
	
	/**
	 * Creates a preference page for bracket setting.
	 */
	public CoedBracketPreferencePage() {
		super();
		
		org.modelrefactoring.evolution.coed.resource.coed.ICoedMetaInformation metaInformation = new org.modelrefactoring.evolution.coed.resource.coed.mopp.CoedMetaInformation();
	...
	public void init(IWorkbench workbench) {
		setPreferenceStore(org.modelrefactoring.evolution.coed.resource.coed.ui.CoedUIPlugin.getDefault().getPreferenceStore());
		setDescription("Define the coloring of matching brackets.");
		
		bracketsTmp = new org.modelrefactoring.evolution.coed.resource.coed.ui.CoedBracketSet();
		for (String languageID : languageIDs) {
			bracketSetTemp.put(languageID, getPreferenceStore().getString(languageID + org.modelrefactoring.evolution.coed.resource.coed.ui.CoedPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		}
	}
	...
	private void handleMatchingBracketsSelection() {
		...
		enableCheckbox.setSelection(getPreferenceStore().getBoolean(org.modelrefactoring.evolution.coed.resource.coed.ui.CoedPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		enableClosingInside.setSelection(false);
		matchingBracketsColorButton.setEnabled(getPreferenceStore().getBoolean(		org.modelrefactoring.evolution.coed.resource.coed.ui.CoedPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		...
		initializeLanguage();
		bracketsTmp.deserialize(getPreferenceStore().getString(language + org.modelrefactoring.evolution.coed.resource.coed.ui.CoedPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
	...
	private void addListenersToStyleButtons() {
		...
		bracketsList.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				org.modelrefactoring.evolution.coed.resource.coed.ICoedBracketPair bracketPair = getSelectedBracketPair();
				if (bracketPair == null) {
					removeBracketButton.setEnabled(false);
					return;
				}
	...
	protected void performDefaults() {
		...
		enableCheckbox.setSelection(preferenceStore.getDefaultBoolean(org.modelrefactoring.evolution.coed.resource.coed.ui.CoedPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		matchingBracketsColorButton.setEnabled(enableCheckbox.getSelection());
		matchingBracketsColorEditor.setColorValue(PreferenceConverter.getDefaultColor(preferenceStore, BRACKETS_COLOR));
		String defaultBrackets = preferenceStore.getDefaultString(language + org.modelrefactoring.evolution.coed.resource.coed.ui.CoedPreferenceConstants.EDITOR_BRACKETS_SUFFIX);
	...
	private void updateActiveEditor() {
		...
		preferenceStore.setValue(org.modelrefactoring.evolution.coed.resource.coed.ui.CoedPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX, enableCheckbox.getSelection());
		preferenceStore.setValue(language + org.modelrefactoring.evolution.coed.resource.coed.ui.CoedPreferenceConstants.EDITOR_BRACKETS_SUFFIX, bracketSetTemp.get(language));
		IWorkbench workbench = org.eclipse.ui.PlatformUI.getWorkbench();
		IEditorPart editor = workbench.getActiveWorkbenchWindow().getActivePage().getActiveEditor();
		if (editor != null && editor instanceof org.modelrefactoring.evolution.coed.resource.coed.ui.CoedEditor) {
			((org.modelrefactoring.evolution.coed.resource.coed.ui.CoedEditor) editor).invalidateTextRepresentation();
		}
	}
	
	protected org.modelrefactoring.evolution.coed.resource.coed.ICoedBracketPair getSelectedBracketPair() {
