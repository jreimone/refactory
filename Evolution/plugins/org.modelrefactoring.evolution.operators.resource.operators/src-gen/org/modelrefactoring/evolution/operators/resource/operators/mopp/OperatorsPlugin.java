 ...
 */
public class OperatorsPlugin extends Plugin {
	
	public static final String PLUGIN_ID = "org.modelrefactoring.evolution.operators.resource.operators";
	/**
	...
	private static OperatorsPlugin plugin;
	
	public OperatorsPlugin() {
		super();
	}
	...
	public static OperatorsPlugin getDefault() {
		return plugin;
	}
	...
	protected static IStatus log(int type, String message, Throwable throwable) {
		IStatus status;
		if (throwable != null) {
			status = new Status(type, OperatorsPlugin.PLUGIN_ID, 0, message, throwable);
		} else {
			status = new Status(type, OperatorsPlugin.PLUGIN_ID, message);
		}
		final OperatorsPlugin pluginInstance = OperatorsPlugin.getDefault();
