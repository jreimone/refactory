...
public class OperatorsSyntaxErrorMessageConverter {
	
	private String[] tokenNames;
	
	public OperatorsSyntaxErrorMessageConverter(String[] tokenNames) {
	 ...
	 */
	public org.modelrefactoring.evolution.operators.resource.operators.mopp.OperatorsLocalizedMessage translateLexicalError(RecognitionException e, List<RecognitionException> lexerExceptions, List<Integer> lexerExceptionPositions)  {
		String message = getMessage(e);
		return new org.modelrefactoring.evolution.operators.resource.operators.mopp.OperatorsLocalizedMessage(message, e.charPositionInLine, e.line, lexerExceptionPositions.get(lexerExceptions.indexOf(e)), lexerExceptionPositions.get(lexerExceptions.indexOf(e)));
	}
	
	/**
	 * Translates errors thrown by the parser into human readable messages.
	 */
	public org.modelrefactoring.evolution.operators.resource.operators.mopp.OperatorsLocalizedMessage translateParseError(RecognitionException e)  {
		String message = getMessage(e);
		
		if (e.token instanceof CommonToken) {
			CommonToken ct = (CommonToken) e.token;
			return new org.modelrefactoring.evolution.operators.resource.operators.mopp.OperatorsLocalizedMessage(message, ct.getCharPositionInLine(), ct.getLine(), ct.getStartIndex(), ct.getStopIndex());
		} else {
			int position = 1;
			int line = 1;
			if (e.token != null) {
			...
			return new org.modelrefactoring.evolution.operators.resource.operators.mopp.OperatorsLocalizedMessage(message, position, line, 1, 5);
		}
	}
	...
	protected String getTokenName(int tokenType)  {
		...
		if (tokenType < 0) {
			...
			tokenName = org.modelrefactoring.evolution.operators.resource.operators.util.OperatorsStringUtil.formatTokenName(tokenName);
		}
		return tokenName;
	}
