...
public class OperatorsDefaultNameProvider implements org.modelrefactoring.evolution.operators.resource.operators.IOperatorsNameProvider {
		...
		if (nameAttr instanceof EAttribute) {
			...
			for (EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					Object result = org.modelrefactoring.evolution.operators.resource.operators.util.OperatorsEObjectUtil.invokeOperation(element, operation);
					if (result != null) {
