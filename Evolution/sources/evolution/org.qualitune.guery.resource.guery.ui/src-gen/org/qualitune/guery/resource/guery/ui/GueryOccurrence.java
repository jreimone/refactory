...
public class GueryOccurrence {
	...
	public final static String OCCURRENCE_ANNOTATION_ID = "org.qualitune.guery.resource.guery.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "org.qualitune.guery.resource.guery.ui.occurences.declaration";
	...
	private org.qualitune.guery.resource.guery.ui.GueryTokenScanner tokenScanner;
	...
	private org.qualitune.guery.resource.guery.IGueryTextResource textResource;
	 ...
	 */
	public GueryOccurrence(org.qualitune.guery.resource.guery.IGueryTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, org.qualitune.guery.resource.guery.ui.GueryTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new org.qualitune.guery.resource.guery.mopp.GueryMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(org.qualitune.guery.resource.guery.ui.GueryBracketSet bracketSet) {
		if (textResource == null) {
			return;
		}
		org.eclipse.swt.custom.StyledText textWidget = projectionViewer.getTextWidget();
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, org.qualitune.guery.resource.guery.ui.GueryPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if ((isNull && elementsAtDefinition.contains(occEO)) || !isNull && definitionElement.equals(occEO)) {
						addAnnotation(document, org.qualitune.guery.resource.guery.ui.GueryPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, org.qualitune.guery.resource.guery.ui.GueryPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == org.qualitune.guery.resource.guery.ui.GueryPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(org.qualitune.guery.resource.guery.ui.GueryOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(org.qualitune.guery.resource.guery.ui.GueryOccurrence.DECLARATION_ANNOTATION_ID);
	}
