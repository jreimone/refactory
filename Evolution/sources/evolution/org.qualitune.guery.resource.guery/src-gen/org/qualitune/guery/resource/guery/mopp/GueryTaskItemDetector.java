...
public class GueryTaskItemDetector {
	...
	public java.util.List<org.qualitune.guery.resource.guery.mopp.GueryTaskItem> findTaskItems(String text, int line, int charStart) {
		java.util.List<org.qualitune.guery.resource.guery.mopp.GueryTaskItem> foundItems = new java.util.ArrayList<org.qualitune.guery.resource.guery.mopp.GueryTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new org.qualitune.guery.resource.guery.mopp.GueryTaskItem(keyword, message, localLine, offset, end));
