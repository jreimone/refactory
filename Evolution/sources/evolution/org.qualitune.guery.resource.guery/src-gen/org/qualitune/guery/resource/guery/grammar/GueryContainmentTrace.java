...
public class GueryContainmentTrace {
	 ...
	 */
	private org.qualitune.guery.resource.guery.mopp.GueryContainedFeature[] path;
	
	public GueryContainmentTrace(org.eclipse.emf.ecore.EClass startClass, org.qualitune.guery.resource.guery.mopp.GueryContainedFeature[] path) {
		super();
		// Verify arguments
		if (startClass != null) {
			if (path.length > 0) {
	...
	public org.qualitune.guery.resource.guery.mopp.GueryContainedFeature[] getPath() {
		return path;
	}
	
	public String toString() {
		return (startClass == null ? "null" : startClass.getName()) + "->" + org.qualitune.guery.resource.guery.util.GueryStringUtil.explode(path, "->");
	}
