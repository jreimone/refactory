...
public class PropertiesDefaultNameProvider implements org.emftext.refactoring.tests.properties.resource.properties.IPropertiesNameProvider {
		...
		if (nameAttr instanceof org.eclipse.emf.ecore.EAttribute) {
			...
			for (org.eclipse.emf.ecore.EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					Object result = org.emftext.refactoring.tests.properties.resource.properties.util.PropertiesEObjectUtil.invokeOperation(element, operation);
					if (result != null) {
