...
public class RefspecHighlighting implements org.eclipse.jface.viewers.ISelectionProvider, org.eclipse.jface.viewers.ISelectionChangedListener {
	...
	private final static org.emftext.language.refactoring.specification.resource.ui.RefspecPositionHelper positionHelper = new org.emftext.language.refactoring.specification.resource.ui.RefspecPositionHelper();
	private boolean isHighlightBrackets = true;
	private boolean isHighlightOccurrences = true;
	private org.emftext.language.refactoring.specification.resource.ui.RefspecTokenScanner scanner;
	...
	private org.emftext.language.refactoring.specification.resource.ui.RefspecOccurrence occurrence;
	private org.emftext.language.refactoring.specification.resource.ui.RefspecBracketSet bracketSet;
	...
	private final class UpdateHighlightingListener implements org.eclipse.swt.events.KeyListener, org.eclipse.swt.events.VerifyListener, org.eclipse.swt.events.MouseListener, org.emftext.language.refactoring.specification.resource.IRefspecBackgroundParsingListener {
	...
	public RefspecHighlighting(org.emftext.language.refactoring.specification.resource.IRefspecTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceviewer, org.emftext.language.refactoring.specification.resource.ui.RefspecColorManager colorManager, org.emftext.language.refactoring.specification.resource.ui.RefspecEditor editor) {
		this.display = org.eclipse.swt.widgets.Display.getCurrent();
		sourceviewer.getSelectionProvider();
		preferenceStore = org.emftext.language.refactoring.specification.resource.mopp.RefspecPlugin.getDefault().getPreferenceStore();
		textWidget = sourceviewer.getTextWidget();
		projectionViewer = sourceviewer;
		scanner = new org.emftext.language.refactoring.specification.resource.ui.RefspecTokenScanner(colorManager);
		occurrence = new org.emftext.language.refactoring.specification.resource.ui.RefspecOccurrence(textResource, sourceviewer, scanner);
		bracketSet = new org.emftext.language.refactoring.specification.resource.ui.RefspecBracketSet(sourceviewer, "refspec");
		this.colorManager = colorManager;
		isHighlightBrackets = preferenceStore.getBoolean(org.emftext.language.refactoring.specification.resource.ui.RefspecPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX);
		isHighlightOccurrences = preferenceStore.getBoolean(org.emftext.language.refactoring.specification.resource.ui.RefspecPreferenceConstants.EDITOR_OCCURRENCE_CHECKBOX);
		definitionColor = colorManager.getColor(org.eclipse.jface.preference.PreferenceConverter.getColor(preferenceStore, org.emftext.language.refactoring.specification.resource.ui.RefspecPreferenceConstants.EDITOR_DEFINITION_COLOR));
		proxyColor = colorManager.getColor(org.eclipse.jface.preference.PreferenceConverter.getColor(preferenceStore, org.emftext.language.refactoring.specification.resource.ui.RefspecPreferenceConstants.EDITOR_PROXY_COLOR));
		bracketColor = colorManager.getColor(org.eclipse.jface.preference.PreferenceConverter.getColor(preferenceStore, org.emftext.language.refactoring.specification.resource.ui.RefspecPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR));
		black = colorManager.getColor(new org.eclipse.swt.graphics.RGB(0, 0, 0));
		
		addListeners(editor);
	}
	
	private void addListeners(org.emftext.language.refactoring.specification.resource.ui.RefspecEditor editor) {
	...
	private void setHighlighting() {
		...
		if (occurrence.isPositionsChanged()) {
			...
			org.emftext.language.refactoring.specification.resource.ui.RefspecPositionCategory.DEFINTION.toString());
			setCategoryHighlighting(document,
			org.emftext.language.refactoring.specification.resource.ui.RefspecPositionCategory.PROXY.toString());
		}
		setCategoryHighlighting(document, org.emftext.language.refactoring.specification.resource.ui.RefspecPositionCategory.BRACKET.toString());
	}
	...
	private void setCategoryHighlighting(org.eclipse.jface.text.IDocument document, String category) {
		...
		if (category.equals(org.emftext.language.refactoring.specification.resource.ui.RefspecPositionCategory.PROXY.toString())) {
		...
		for (org.eclipse.jface.text.Position position : positions) {
			...
			if (tmpPosition != null) {
				if (category.equals(org.emftext.language.refactoring.specification.resource.ui.RefspecPositionCategory.DEFINTION.toString())) {
				...
				if (category.equals(org.emftext.language.refactoring.specification.resource.ui.RefspecPositionCategory.BRACKET.toString())) {
					styleRange = getStyleRangeAtPosition(tmpPosition);
	...
	private void removeHighlighting() {
		org.eclipse.jface.text.IDocument document = projectionViewer.getDocument();
		removeHighlightingCategory(document, org.emftext.language.refactoring.specification.resource.ui.RefspecPositionCategory.BRACKET.toString());
		if (occurrence.isToRemoveHighlighting()) {
			removeHighlightingCategory(document, org.emftext.language.refactoring.specification.resource.ui.RefspecPositionCategory.DEFINTION.toString());
			removeHighlightingCategory(document, org.emftext.language.refactoring.specification.resource.ui.RefspecPositionCategory.PROXY.toString());
		}
	}
	
	private void removeHighlightingCategory(org.eclipse.jface.text.IDocument document, String category) {
		org.eclipse.jface.text.Position[] positions = positionHelper.getPositions(document, category);
		boolean isOccurrence = (category.equals(org.emftext.language.refactoring.specification.resource.ui.RefspecPositionCategory.DEFINTION.toString()) || category.equals(org.emftext.language.refactoring.specification.resource.ui.RefspecPositionCategory.PROXY.toString()));
		if (category.equals(org.emftext.language.refactoring.specification.resource.ui.RefspecPositionCategory.BRACKET.toString())) {
		...
		display.syncExec(new Runnable() {
			public void run() {
				...
				if (selectedEObject != null) {
					setSelection(new org.emftext.language.refactoring.specification.resource.ui.RefspecEObjectSelection(selectedEObject, false));
				}
			}
		});
	}
	...
	private void handleContentOutlineSelection(org.eclipse.jface.viewers.ISelection selection) {
		if (!selection.isEmpty()) {
			...
			if (selectedElement instanceof org.eclipse.emf.ecore.EObject) {
				...
				if (resource instanceof org.emftext.language.refactoring.specification.resource.IRefspecTextResource) {
