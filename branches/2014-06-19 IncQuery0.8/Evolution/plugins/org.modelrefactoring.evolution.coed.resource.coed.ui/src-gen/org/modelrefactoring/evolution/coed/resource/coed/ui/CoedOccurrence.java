 ...
 */
public class CoedOccurrence {
	
	private static interface ITokenScannerConstraint {
		public boolean mustStop(org.modelrefactoring.evolution.coed.resource.coed.ui.ICoedTokenScanner tokenScanner);
	}
	
	public final static String OCCURRENCE_ANNOTATION_ID = "org.modelrefactoring.evolution.coed.resource.coed.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "org.modelrefactoring.evolution.coed.resource.coed.ui.occurences.declaration";
	...
	/**
	 * Creates a CoedOccurrence object to find positions to highlight.
	 * 
	 * @param textResource the text resource for location
	 * @param projectionViewer the viewer for the text
	 */
	public CoedOccurrence(org.modelrefactoring.evolution.coed.resource.coed.ICoedTextResource textResource, ProjectionViewer projectionViewer) {
		super();
	...
	protected int getLength(org.modelrefactoring.evolution.coed.resource.coed.ICoedLocationMap locationMap, EObject eObject) {
	...
	public void updateOccurrenceAnnotations() {
		...
		org.modelrefactoring.evolution.coed.resource.coed.ui.ICoedTokenScanner tokenScanner = scan(referencedElement, new ITokenScannerConstraint() {
	...
	protected void addAnnotationsForReferences(EObject referencedElement, List<String> matchingNames) {
		...
		for (org.eclipse.emf.ecore.EStructuralFeature.Setting setting : referencingObjects) {
			...
			for (String name : matchingNames) {
				...
				if (index > 0) {
					addAnnotation(document, org.modelrefactoring.evolution.coed.resource.coed.ui.CoedPositionCategory.PROXY, name, index, name.length());
				}
			}
		}
	}
	...
	protected List<String> addAnnotationsForDefinition(EObject referencedElement, String definitionText, int definitionOffset, final int caretOffset) {
		...
		if (definitionText == null) {
			...
			final List<String> names = new org.modelrefactoring.evolution.coed.resource.coed.analysis.CoedDefaultNameProvider().getNames(referencedElement);
			scan(referencedElement, new ITokenScannerConstraint() {
				...
				public boolean mustStop(org.modelrefactoring.evolution.coed.resource.coed.ui.ICoedTokenScanner tokenScanner) {
					...
					if (names.contains(text) && isContainedIn(offset, length, caretOffset)) {
						...
						addAnnotation(document, org.modelrefactoring.evolution.coed.resource.coed.ui.CoedPositionCategory.DEFINITION, text, offset, text.length());
					}
					return false;
				}
			});
			...
			addAnnotation(document, org.modelrefactoring.evolution.coed.resource.coed.ui.CoedPositionCategory.DEFINITION, definitionText, definitionOffset, definitionText.length());
	...
	protected int getIndexOf(EObject eObject, final String text) {
		org.modelrefactoring.evolution.coed.resource.coed.ui.ICoedTokenScanner tokenScanner = scan(eObject, new ITokenScannerConstraint() {
	...
	protected org.modelrefactoring.evolution.coed.resource.coed.ui.ICoedTokenScanner scan(EObject object, ITokenScannerConstraint constraint) {
		IDocument document = getSourceViewer().getDocument();
		
		org.modelrefactoring.evolution.coed.resource.coed.ICoedLocationMap locationMap = textResource.getLocationMap();
		
		org.modelrefactoring.evolution.coed.resource.coed.ui.ICoedTokenScanner tokenScanner = createTokenScanner();
	...
	protected org.modelrefactoring.evolution.coed.resource.coed.ui.ICoedTokenScanner createTokenScanner() {
		return new org.modelrefactoring.evolution.coed.resource.coed.ui.CoedUIMetaInformation().createTokenScanner(null, null);
	}
	
	protected void addAnnotation(IDocument document, org.modelrefactoring.evolution.coed.resource.coed.ui.CoedPositionCategory type, String text, int offset, int length) {
		...
		if (type == org.modelrefactoring.evolution.coed.resource.coed.ui.CoedPositionCategory.DEFINITION) {
	...
	protected void removeAnnotations() {
		removeAnnotations(org.modelrefactoring.evolution.coed.resource.coed.ui.CoedOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(org.modelrefactoring.evolution.coed.resource.coed.ui.CoedOccurrence.DECLARATION_ANNOTATION_ID);
	}
