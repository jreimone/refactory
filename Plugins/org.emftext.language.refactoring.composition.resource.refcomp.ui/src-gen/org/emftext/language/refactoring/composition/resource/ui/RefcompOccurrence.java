 ...
 */
public class RefcompOccurrence {
	
	private static interface ITokenScannerConstraint {
		public boolean mustStop(org.emftext.language.refactoring.composition.resource.ui.IRefcompTokenScanner tokenScanner);
	}
	
	public final static String OCCURRENCE_ANNOTATION_ID = "org.emftext.language.refactoring.composition.resource.refcomp.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "org.emftext.language.refactoring.composition.resource.refcomp.ui.occurences.declaration";
	
	private final static org.emftext.language.refactoring.composition.resource.ui.RefcompPositionHelper positionHelper = new org.emftext.language.refactoring.composition.resource.ui.RefcompPositionHelper();
	...
	/**
	 * <p>
	 * Creates a RefcompOccurrence object to find positions to highlight.
	 * </p>
	 * 
	 * @param textResource the text resource for location
	 * @param projectionViewer the viewer for the text
	 */
	public RefcompOccurrence(org.emftext.language.refactoring.composition.resource.IRefcompTextResource textResource, ProjectionViewer projectionViewer) {
		super();
	...
	protected int getLength(org.emftext.language.refactoring.composition.resource.IRefcompLocationMap locationMap, EObject eObject) {
	...
	public void updateOccurrenceAnnotations() {
		...
		org.emftext.language.refactoring.composition.resource.ui.IRefcompTokenScanner tokenScanner = scan(referencedElement, new ITokenScannerConstraint() {
	...
	protected void addAnnotationsForReferences(EObject referencedElement, List<String> matchingNames) {
		...
		for (Setting setting : referencingObjects) {
			...
			for (String name : matchingNames) {
				...
				if (index > 0) {
					addAnnotation(document, org.emftext.language.refactoring.composition.resource.ui.RefcompPositionCategory.PROXY, name, index, name.length());
				}
			}
		}
	}
	...
	protected List<String> addAnnotationsForDefinition(EObject referencedElement, String definitionText, int definitionOffset, final int caretOffset) {
		...
		if (definitionText == null) {
			...
			final List<String> names = new org.emftext.language.refactoring.composition.resource.analysis.RefcompDefaultNameProvider().getNames(referencedElement);
			scan(referencedElement, new ITokenScannerConstraint() {
				...
				public boolean mustStop(org.emftext.language.refactoring.composition.resource.ui.IRefcompTokenScanner tokenScanner) {
					...
					if (names.contains(text) && isContainedIn(offset, length, caretOffset)) {
						...
						addAnnotation(document, org.emftext.language.refactoring.composition.resource.ui.RefcompPositionCategory.DEFINITION, text, offset, text.length());
					}
					return false;
				}
			});
			...
			addAnnotation(document, org.emftext.language.refactoring.composition.resource.ui.RefcompPositionCategory.DEFINITION, definitionText, definitionOffset, definitionText.length());
	...
	protected int getIndexOf(EObject eObject, final String text) {
		org.emftext.language.refactoring.composition.resource.ui.IRefcompTokenScanner tokenScanner = scan(eObject, new ITokenScannerConstraint() {
	...
	protected org.emftext.language.refactoring.composition.resource.ui.IRefcompTokenScanner scan(EObject object, ITokenScannerConstraint constraint) {
		IDocument document = getSourceViewer().getDocument();
		
		org.emftext.language.refactoring.composition.resource.IRefcompLocationMap locationMap = textResource.getLocationMap();
		
		org.emftext.language.refactoring.composition.resource.ui.IRefcompTokenScanner tokenScanner = createTokenScanner();
	...
	protected org.emftext.language.refactoring.composition.resource.ui.IRefcompTokenScanner createTokenScanner() {
		return new org.emftext.language.refactoring.composition.resource.ui.RefcompUIMetaInformation().createTokenScanner(null, null);
	}
	
	protected void addAnnotation(IDocument document, org.emftext.language.refactoring.composition.resource.ui.RefcompPositionCategory type, String text, int offset, int length) {
		...
		if (type == org.emftext.language.refactoring.composition.resource.ui.RefcompPositionCategory.DEFINITION) {
	...
	protected void removeAnnotations() {
		removeAnnotations(org.emftext.language.refactoring.composition.resource.ui.RefcompOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(org.emftext.language.refactoring.composition.resource.ui.RefcompOccurrence.DECLARATION_ANNOTATION_ID);
	}
