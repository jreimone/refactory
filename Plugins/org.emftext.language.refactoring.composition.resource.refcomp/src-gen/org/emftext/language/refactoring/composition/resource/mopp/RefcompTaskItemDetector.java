...
/**
 * The RefcompTaskItemDetector is used to find task items in text documents. The
 * current implementation searches for specific keywords to detect task items. The
 * RefcompTaskItemDetector is used both by the TaskItemBuilder and the editor.
 */
public class RefcompTaskItemDetector {
	...
	public List<org.emftext.language.refactoring.composition.resource.mopp.RefcompTaskItem> findTaskItems(String text, int line, int charStart) {
		List<org.emftext.language.refactoring.composition.resource.mopp.RefcompTaskItem> foundItems = new ArrayList<org.emftext.language.refactoring.composition.resource.mopp.RefcompTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new org.emftext.language.refactoring.composition.resource.mopp.RefcompTaskItem(keyword, message, localLine, offset, end));
