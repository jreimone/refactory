 ...
 */
public class RefcompPlugin extends Plugin {
	
	public static final String PLUGIN_ID = "org.emftext.language.refactoring.composition.resource.refcomp";
	/**
	...
	private static RefcompPlugin plugin;
	
	public RefcompPlugin() {
		super();
	}
	...
	public static RefcompPlugin getDefault() {
		return plugin;
	}
	...
	protected static IStatus log(int type, String message, Throwable throwable) {
		IStatus status;
		if (throwable != null) {
			status = new Status(type, RefcompPlugin.PLUGIN_ID, 0, message, throwable);
		} else {
			status = new Status(type, RefcompPlugin.PLUGIN_ID, message);
		}
		final RefcompPlugin pluginInstance = RefcompPlugin.getDefault();
