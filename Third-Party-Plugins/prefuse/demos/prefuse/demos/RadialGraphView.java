...
import prefuse.demos.TreeMap.FillColorAction;
...
import prefuse.visualization.MultiEdgeRenderer;
...
public class RadialGraphView extends Display {
	...
	public RadialGraphView(Graph g, String label, String edgeTypePredicate, Map<String, String> edgeTypeColorMap, boolean useStraightLineForSingleEdges) {
		...
		m_edgeRenderer = new MultiEdgeRenderer(useStraightLineForSingleEdges);
		...
		List<ColorAction> edgeColors = new ArrayList<ColorAction>();
		if(edgeTypeColorMap != null){
			if(edgeTypeColorMap != null){
				for (Entry<String, String> entry : edgeTypeColorMap.entrySet()) {
					String key = entry.getKey();
					String colorString = entry.getValue();
					String[] colorValues = colorString.split(",");
					if(colorValues.length == 3 && edgeTypePredicate != null && key != null){
						try {
							ColorAction edgeColor = new ColorAction(treeEdges, VisualItem.STROKECOLOR
									, ColorLib.rgb(Integer.parseInt(colorValues[0]),Integer.parseInt(colorValues[1]),Integer.parseInt(colorValues[2])));
							Predicate predicate = ExpressionParser.predicate(edgeTypePredicate + " = '" + key + "'");
							edgeColor.setFilterPredicate(predicate);
		...
		for (ColorAction edgeColor : edgeColors) {
	...
	public static JPanel demo(Graph g, final String label) {        
		...
		final RadialGraphView gview = new RadialGraphView(g, label, null, null, false);
