 ...
 */
public class XwpfOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "dk.itu.sdg.language.xwpf.resource.xwpf.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "dk.itu.sdg.language.xwpf.resource.xwpf.ui.occurences.declaration";
	...
	private dk.itu.sdg.language.xwpf.resource.xwpf.ui.XwpfTokenScanner tokenScanner;
	...
	private dk.itu.sdg.language.xwpf.resource.xwpf.IXwpfTextResource textResource;
	 ...
	 */
	public XwpfOccurrence(dk.itu.sdg.language.xwpf.resource.xwpf.IXwpfTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, dk.itu.sdg.language.xwpf.resource.xwpf.ui.XwpfTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new dk.itu.sdg.language.xwpf.resource.xwpf.mopp.XwpfMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(dk.itu.sdg.language.xwpf.resource.xwpf.ui.XwpfBracketSet bracketSet) {
		if (textResource == null) {
			return;
		}
		org.eclipse.swt.custom.StyledText textWidget = projectionViewer.getTextWidget();
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, dk.itu.sdg.language.xwpf.resource.xwpf.ui.XwpfPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if (elementsAtDefinition.contains(occEO) || definitionElement.equals(occEO)) {
						addAnnotation(document, dk.itu.sdg.language.xwpf.resource.xwpf.ui.XwpfPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, dk.itu.sdg.language.xwpf.resource.xwpf.ui.XwpfPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == dk.itu.sdg.language.xwpf.resource.xwpf.ui.XwpfPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(dk.itu.sdg.language.xwpf.resource.xwpf.ui.XwpfOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(dk.itu.sdg.language.xwpf.resource.xwpf.ui.XwpfOccurrence.DECLARATION_ANNOTATION_ID);
	}
