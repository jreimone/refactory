...
public class XwpfDefaultNameProvider implements dk.itu.sdg.language.xwpf.resource.xwpf.IXwpfNameProvider {
		...
		if (nameAttr instanceof org.eclipse.emf.ecore.EAttribute) {
			...
			for (org.eclipse.emf.ecore.EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					Object result = dk.itu.sdg.language.xwpf.resource.xwpf.util.XwpfEObjectUtil.invokeOperation(element, operation);
					if (result != null) {
