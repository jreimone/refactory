...
public class RefspecOccurrence {
	...
	private org.emftext.language.refactoring.refactoring_specification.resource.refspec.ui.RefspecTokenScanner tokenScanner;
	...
	//
	public RefspecOccurrence(org.emftext.language.refactoring.refactoring_specification.resource.refspec.IRefspecTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, org.emftext.language.refactoring.refactoring_specification.resource.refspec.ui.RefspecTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new org.emftext.language.refactoring.refactoring_specification.resource.refspec.mopp.RefspecMetaInformation().getTokenNames();
	...
	public void handleOccurrenceHighlighting(org.emftext.language.refactoring.refactoring_specification.resource.refspec.ui.RefspecBracketSet bracketSet) {
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addPosition(document, org.emftext.language.refactoring.refactoring_specification.resource.refspec.ui.RefspecPositionCategory.DEFINTION.toString());
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if ((isNull && elementsAtDefinition.contains(occEO)) || !isNull && definitionElement.equals(occEO)) {
						addPosition(document, org.emftext.language.refactoring.refactoring_specification.resource.refspec.ui.RefspecPositionCategory.PROXY.toString());
					}
				}
			}
			token = tokenScanner.nextToken();
