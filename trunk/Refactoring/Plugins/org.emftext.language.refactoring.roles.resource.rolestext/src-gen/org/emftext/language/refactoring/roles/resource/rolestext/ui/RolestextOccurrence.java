...
public class RolestextOccurrence {
	...
	private org.emftext.language.refactoring.roles.resource.rolestext.ui.RolestextTokenScanner tokenScanner;
	...
	//
	public RolestextOccurrence(org.emftext.language.refactoring.roles.resource.rolestext.IRolestextTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, org.emftext.language.refactoring.roles.resource.rolestext.ui.RolestextTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new org.emftext.language.refactoring.roles.resource.rolestext.mopp.RolestextMetaInformation().getTokenNames();
	...
	public void handleOccurrenceHighlighting(org.emftext.language.refactoring.roles.resource.rolestext.ui.RolestextBracketSet bracketSet) {
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addPosition(document, org.emftext.language.refactoring.roles.resource.rolestext.ui.RolestextPositionCategory.DEFINTION.toString());
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if ((isNull && elementsAtDefinition.contains(occEO)) || !isNull && definitionElement.equals(occEO)) {
						addPosition(document, org.emftext.language.refactoring.roles.resource.rolestext.ui.RolestextPositionCategory.PROXY.toString());
					}
				}
			}
			token = tokenScanner.nextToken();
