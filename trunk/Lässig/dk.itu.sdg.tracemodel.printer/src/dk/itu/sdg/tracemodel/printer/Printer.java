...
public class Printer {
	...
	public void toTraceModel() {
		...
		String tracemodelPath = "output/tracemodel.model";

		Bundle bundle = Activator.getDefault().getBundle();

		String loc1 = bundle.getLocation();
		String pluginPath = loc1.replace("reference:file:/", "");
		URI uri = URI.createFileURI(pluginPath + tracemodelPath);

		Resource.Factory.Registry.INSTANCE.getExtensionToFactoryMap().put("model", new XMIResourceFactoryImpl());
		ResourceSet rs = new ResourceSetImpl();
		Resource resource = rs.createResource(uri);

		SimpleTraceFactory factory = SimpleTraceFactory.eINSTANCE;
