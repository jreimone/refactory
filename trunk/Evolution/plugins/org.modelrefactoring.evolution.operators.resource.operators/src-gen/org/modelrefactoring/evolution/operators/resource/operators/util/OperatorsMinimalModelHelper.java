 ...
 */
public class OperatorsMinimalModelHelper {
	
	private final static org.modelrefactoring.evolution.operators.resource.operators.util.OperatorsEClassUtil eClassUtil = new org.modelrefactoring.evolution.operators.resource.operators.util.OperatorsEClassUtil();
			...
			if (feature instanceof EReference) {
				...
				if (type instanceof EClass) {
					...
					for (int i = 0; i < lowerBound; i++) {
						...
						if (reference.isContainment()) {
							...
							String initialValue = "#some" + org.modelrefactoring.evolution.operators.resource.operators.util.OperatorsStringUtil.capitalize(typeClass.getName());
						...
						if (value instanceof List<?>) {
							List<EObject> list = org.modelrefactoring.evolution.operators.resource.operators.util.OperatorsListUtil.castListUnchecked(value);
							list.add(subModel);
						} else {
				...
				if ("EString".equals(attribute.getEType().getName())) {
					...
					if (attribute.getName().equals("name") && name != null) {
						...
						initialValue = "some" + org.modelrefactoring.evolution.operators.resource.operators.util.OperatorsStringUtil.capitalize(attribute.getName());
					}
					Object value = root.eGet(attribute);
					if (value instanceof List<?>) {
						List<String> list = org.modelrefactoring.evolution.operators.resource.operators.util.OperatorsListUtil.castListUnchecked(value);
