...
/**
 * The OperatorsTaskItemDetector is used to find task items in text documents. The
 * current implementation searches for specific keywords to detect task items. The
 * OperatorsTaskItemDetector is used both by the TaskItemBuilder and the editor.
 */
public class OperatorsTaskItemDetector {
	
	public static String[] TASK_ITEM_KEYWORDS = new String[] {"TODO", "FIXME", "XXX"};
	
	public List<org.modelrefactoring.evolution.operators.resource.operators.mopp.OperatorsTaskItem> findTaskItems(String text, int line, int charStart) {
		java.util.List<org.modelrefactoring.evolution.operators.resource.operators.mopp.OperatorsTaskItem> foundItems = new java.util.ArrayList<org.modelrefactoring.evolution.operators.resource.operators.mopp.OperatorsTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new org.modelrefactoring.evolution.operators.resource.operators.mopp.OperatorsTaskItem(keyword, message, localLine, offset, end));
