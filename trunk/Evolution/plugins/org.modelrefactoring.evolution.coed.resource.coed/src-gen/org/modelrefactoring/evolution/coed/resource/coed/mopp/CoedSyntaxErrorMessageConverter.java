...
public class CoedSyntaxErrorMessageConverter {
	
	private String[] tokenNames;
	
	public CoedSyntaxErrorMessageConverter(String[] tokenNames) {
	 ...
	 */
	public org.modelrefactoring.evolution.coed.resource.coed.mopp.CoedLocalizedMessage translateLexicalError(RecognitionException e, List<RecognitionException> lexerExceptions, List<Integer> lexerExceptionPositions)  {
		String message = getMessage(e);
		return new org.modelrefactoring.evolution.coed.resource.coed.mopp.CoedLocalizedMessage(message, e.charPositionInLine, e.line, lexerExceptionPositions.get(lexerExceptions.indexOf(e)), lexerExceptionPositions.get(lexerExceptions.indexOf(e)));
	}
	
	/**
	 * Translates errors thrown by the parser into human readable messages.
	 */
	public org.modelrefactoring.evolution.coed.resource.coed.mopp.CoedLocalizedMessage translateParseError(RecognitionException e)  {
		String message = getMessage(e);
		
		if (e.token instanceof CommonToken) {
			CommonToken ct = (CommonToken) e.token;
			return new org.modelrefactoring.evolution.coed.resource.coed.mopp.CoedLocalizedMessage(message, ct.getCharPositionInLine(), ct.getLine(), ct.getStartIndex(), ct.getStopIndex());
		} else {
			int position = 1;
			int line = 1;
			if (e.token != null) {
			...
			return new org.modelrefactoring.evolution.coed.resource.coed.mopp.CoedLocalizedMessage(message, position, line, 1, 5);
		}
	}
	...
	protected String getTokenName(int tokenType)  {
		...
		if (tokenType < 0) {
			...
			tokenName = org.modelrefactoring.evolution.coed.resource.coed.util.CoedStringUtil.formatTokenName(tokenName);
		}
		return tokenName;
	}
