...
public class GueryMinimalModelHelper {
	...
	private final static org.qualitune.guery.resource.guery.util.GueryEClassUtil eClassUtil = new org.qualitune.guery.resource.guery.util.GueryEClassUtil();
			...
			if (feature instanceof org.eclipse.emf.ecore.EReference) {
				...
				if (type instanceof org.eclipse.emf.ecore.EClass) {
					...
					for (int i = 0; i < lowerBound; i++) {
						...
						if (reference.isContainment()) {
							...
							String initialValue = "#some" + org.qualitune.guery.resource.guery.util.GueryStringUtil.capitalize(typeClass.getName());
						...
						if (value instanceof java.util.List<?>) {
							java.util.List<org.eclipse.emf.ecore.EObject> list = org.qualitune.guery.resource.guery.util.GueryListUtil.castListUnchecked(value);
							list.add(subModel);
						} else {
				...
				if ("EString".equals(attribute.getEType().getName())) {
					...
					if (attribute.getName().equals("name") && name != null) {
						...
						initialValue = "some" + org.qualitune.guery.resource.guery.util.GueryStringUtil.capitalize(attribute.getName());
					}
					Object value = root.eGet(attribute);
					if (value instanceof java.util.List<?>) {
						java.util.List<String> list = org.qualitune.guery.resource.guery.util.GueryListUtil.castListUnchecked(value);
