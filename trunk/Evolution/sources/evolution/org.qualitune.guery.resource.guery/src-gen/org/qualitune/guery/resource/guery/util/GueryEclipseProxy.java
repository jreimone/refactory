...
public class GueryEclipseProxy {
	...
	public void getDefaultLoadOptionProviderExtensions(java.util.Map<Object, Object> optionsMap) {
		if (org.eclipse.core.runtime.Platform.isRunning()) {
			...
			org.eclipse.core.runtime.IConfigurationElement configurationElements[] = extensionRegistry.getConfigurationElementsFor(org.qualitune.guery.resource.guery.mopp.GueryPlugin.EP_DEFAULT_LOAD_OPTIONS_ID);
			for (org.eclipse.core.runtime.IConfigurationElement element : configurationElements) {
				try {
					org.qualitune.guery.resource.guery.IGueryOptionProvider provider = (org.qualitune.guery.resource.guery.IGueryOptionProvider) element.createExecutableExtension("class");
					final java.util.Map<?, ?> options = provider.getOptions();
					final java.util.Collection<?> keys = options.keySet();
					for (Object key : keys) {
						org.qualitune.guery.resource.guery.util.GueryMapUtil.putAndMergeKeys(optionsMap, key, options.get(key));
					}
				} catch (org.eclipse.core.runtime.CoreException ce) {
					new org.qualitune.guery.resource.guery.util.GueryRuntimeUtil().logError("Exception while getting default options.", ce);
				}
			}
		}
	}
	...
	public void getResourceFactoryExtensions(java.util.Map<String, org.eclipse.emf.ecore.resource.Resource.Factory> factories) {
		if (org.eclipse.core.runtime.Platform.isRunning()) {
			org.eclipse.core.runtime.IExtensionRegistry extensionRegistry = org.eclipse.core.runtime.Platform.getExtensionRegistry();
			org.eclipse.core.runtime.IConfigurationElement configurationElements[] = extensionRegistry.getConfigurationElementsFor(org.qualitune.guery.resource.guery.mopp.GueryPlugin.EP_ADDITIONAL_EXTENSION_PARSER_ID);
			for (org.eclipse.core.runtime.IConfigurationElement element : configurationElements) {
				try {
	 ...
	 */
	public org.qualitune.guery.resource.guery.mopp.GueryResource getResource(org.eclipse.core.resources.IFile file) {
	...
	public void checkEMFValidationConstraints(org.qualitune.guery.resource.guery.IGueryTextResource resource, org.eclipse.emf.ecore.EObject root) {
		...
		if (new org.qualitune.guery.resource.guery.util.GueryRuntimeUtil().isEclipsePlatformRunning()) {
			...
			if (org.eclipse.emf.validation.internal.EMFModelValidationPlugin.getPlugin() != null) {
				try {
					...
					new org.qualitune.guery.resource.guery.util.GueryRuntimeUtil().logError("Exception while checking contraints provided by EMF validator classes.", t);
				}
			}
		}
	}
	
	public void addStatus(org.eclipse.core.runtime.IStatus status, org.qualitune.guery.resource.guery.IGueryTextResource resource, org.eclipse.emf.ecore.EObject root) {
		...
		if (!status.isMultiStatus() || !hasChildren) {
			if (status.getSeverity() == org.eclipse.core.runtime.IStatus.ERROR) {
				for (org.eclipse.emf.ecore.EObject cause : causes) {
					resource.addError(status.getMessage(), org.qualitune.guery.resource.guery.GueryEProblemType.ANALYSIS_PROBLEM, cause);
				}
			}
			if (status.getSeverity() == org.eclipse.core.runtime.IStatus.WARNING) {
				for (org.eclipse.emf.ecore.EObject cause : causes) {
					resource.addWarning(status.getMessage(), org.qualitune.guery.resource.guery.GueryEProblemType.ANALYSIS_PROBLEM, cause);
				}
			}
		}
