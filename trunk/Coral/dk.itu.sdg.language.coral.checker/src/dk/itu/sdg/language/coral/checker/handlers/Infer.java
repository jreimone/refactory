...
import dk.itu.sdg.language.coral.generators.CoralGenerator;
	...
	public void inferFromProject(final Project project) {
		...
		Job job = new Job("Infer Relations from Project") {
			...
			protected IStatus run(IProgressMonitor monitor) {
				...
				CoralGenerator generator = new CoralGenerator();
				InferedCoral inferedCoral = null;
				...
				for (int i = 0; i < numberOfLanguages - 1; i++) {
					...
					for (int j = i + 1; j < numberOfLanguages; j++) {
						...
						for (File fstFile : filesPerLanguageA) {
							...
							if (fstReader.getText() == null) {
								System.out.println("Skipped file combinations with... " + fstFile.getFullPath());
								break;
							}
							
							for (File sndFile : filesPerLanguageB) {
								GReader sndReader = new GReader(sndFile);
								if (sndReader.getText() == null) {
									System.out.println("Skipped file combinations with... " + sndFile.getFullPath());
									break;
								}
								...
								CommonSubStrings commonsSubs = new CommonSubStrings();
									...
									System.out.println("Working on... " + fstFile.getFullPath() + " -- " + sndFile.getFullPath());
									...
									inferedCoral = pathFinder.findCommonContentsElements(fstReader.getEmfResource(), sndReader.getEmfResource(), null, inferedCoral);
								
//								fstReader.getEmfResource().getURI().fileExtension();
								monitor.worked(1);
							}
						}
					}
				...
				generator.setData(inferedCoral);
				generator.generate();
//				generator.generate(languageTypes();
		...
		if (fstFile != null && sndFile != null ) {
			...
			Job job = new Job("Infer Relations from Two Files") {
				...
				protected IStatus run(IProgressMonitor monitor) {
					...
					pathFinder.findCommonContentsOffsets(fstFileAsString, sndFileAsString, null);
					InferedCoral inferedCoral = pathFinder.findCommonContentsElements(reader.getEmfResources().fst, reader.getEmfResources().snd, null, null);
