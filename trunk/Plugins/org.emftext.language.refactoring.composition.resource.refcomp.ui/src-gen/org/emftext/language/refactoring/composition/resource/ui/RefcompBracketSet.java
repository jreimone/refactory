 ...
 */
public class RefcompBracketSet {
	...
	private ArrayList<org.emftext.language.refactoring.composition.resource.IRefcompBracketPair> bracketPairs;
	private String languageID;
	
	/**
	 * Creates a new bracket set to manage bracket pairs.
	 */
	public RefcompBracketSet() {
		super();
		this.languageID = new org.emftext.language.refactoring.composition.resource.mopp.RefcompMetaInformation().getSyntaxName();
		this.bracketPairs = new ArrayList<org.emftext.language.refactoring.composition.resource.IRefcompBracketPair>();
	}
	...
	public org.emftext.language.refactoring.composition.resource.IRefcompBracketPair getBracketPair(int index) {
		try {
	 ...
	 */
	public org.emftext.language.refactoring.composition.resource.IRefcompBracketPair getBracketPair(String opening, String closing) {
		for (org.emftext.language.refactoring.composition.resource.IRefcompBracketPair bracketPair : bracketPairs) {
	...
	public boolean addBracketPair(String opening, String closing, boolean closingEnabledInside, boolean closeAfterEnter) {
		...
		bracketPairs.add(new org.emftext.language.refactoring.composition.resource.mopp.RefcompBracketPair(opening, closing, closingEnabledInside, closeAfterEnter));
		return true;
	}
	...
	public boolean resetBrackets(IPreferenceStore preferenceStore) {
		String bracketPairs = preferenceStore.getString(languageID + org.emftext.language.refactoring.composition.resource.ui.RefcompPreferenceConstants.EDITOR_BRACKETS_SUFFIX);
	 ...
	 */
	public org.emftext.language.refactoring.composition.resource.IRefcompBracketPair remove(String opening, String closing) {
		for (org.emftext.language.refactoring.composition.resource.IRefcompBracketPair bracketPair : bracketPairs) {
			if (bracketPair.getOpeningBracket().equals(opening) && bracketPair.getClosingBracket().equals(closing)) {
	...
	public void deserialize(String bracketSet) {
		bracketPairs = new ArrayList<org.emftext.language.refactoring.composition.resource.IRefcompBracketPair>();
	...
	public void highlightBrackets(IDocument document, int position, int caretOffset) {
		if (position != -1 && position != document.getLength()) {
			positionHelper.addPosition(document, org.emftext.language.refactoring.composition.resource.ui.RefcompPositionCategory.BRACKET.toString(), position, 1);
			positionHelper.addPosition(document, org.emftext.language.refactoring.composition.resource.ui.RefcompPositionCategory.BRACKET.toString(), caretOffset - 1, 1);
		}
	}
