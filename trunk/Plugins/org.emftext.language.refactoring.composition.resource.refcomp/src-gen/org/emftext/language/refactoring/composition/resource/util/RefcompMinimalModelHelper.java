 ...
 */
public class RefcompMinimalModelHelper {
	
	private final static org.emftext.language.refactoring.composition.resource.util.RefcompEClassUtil eClassUtil = new org.emftext.language.refactoring.composition.resource.util.RefcompEClassUtil();
			...
			if (feature instanceof EReference) {
				...
				if (type instanceof EClass) {
					...
					for (int i = 0; i < lowerBound; i++) {
						...
						if (reference.isContainment()) {
							...
							String initialValue = "#some" + org.emftext.language.refactoring.composition.resource.util.RefcompStringUtil.capitalize(typeClass.getName());
						...
						if (value instanceof List<?>) {
							List<EObject> list = org.emftext.language.refactoring.composition.resource.util.RefcompListUtil.castListUnchecked(value);
							list.add(subModel);
						} else {
				...
				if ("EString".equals(attribute.getEType().getName())) {
					...
					if (attribute.getName().equals("name") && name != null) {
						...
						initialValue = "some" + org.emftext.language.refactoring.composition.resource.util.RefcompStringUtil.capitalize(attribute.getName());
					}
					Object value = root.eGet(attribute);
					if (value instanceof List<?>) {
						List<String> list = org.emftext.language.refactoring.composition.resource.util.RefcompListUtil.castListUnchecked(value);
