...
public class RefcompDefaultNameProvider implements org.emftext.language.refactoring.composition.resource.IRefcompNameProvider {
		...
		if (nameAttr instanceof EAttribute) {
			...
			for (EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					Object result = org.emftext.language.refactoring.composition.resource.util.RefcompEObjectUtil.invokeOperation(element, operation);
					if (result != null) {
